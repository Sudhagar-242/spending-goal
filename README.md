...
To authenticate and query data you can use the `shopify` const that is exported from `/app/models/shopify.server.js`:
...
1. Setting up your Shopify app in [/app/models/shopify.server.ts](https://github.com/Shopify/shopify-app-template-remix/blob/main/app/models/shopify.server.ts)
...
2. Pass `isEmbeddedApp: false` to `shopifyApp()` in `./app/models/shopify.server.js|ts`.
